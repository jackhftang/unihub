...
// helper
function toArray(x) {
    if (Array.isArray(x)) return x;else if (typeof x === 'undefined' || x === null) return [];
    return [x];
}
///////////////////////////////////////////////////////////
// TagSet
var TagSet = function () {
    ...
    TagSet.prototype.add = function (obj, tags) {
        ...
        // add to set
        this._bag[id] = {
            tags: StringSet_1.fromArray(arr),
            val: obj
        };
        // add to tags
    ...
    TagSet.prototype.addTag = function (id, tag) {
        ...
        if (s) s.add(id);else this._tags[tag] = StringSet_1.singleton(id);
        return true;
    };
    TagSet.prototype.removeTag = function (id, tag) {
        ...
        if (s) {
            s.remove(id);
            if (s.cardinal === 0) delete this._tags[tag];
            return true;
        } else {
            return false;
        }
    ...
    TagSet.prototype.get = function (id) {
        var x = this._bag[id];
        if (x) return x.val; // allow to modify value
        return null;
    };
    ...
    TagSet.prototype.getAll = function () {
        ...
        return keys.map(function (key) {
            return self._bag[key].val;
        });
    };
    TagSet.prototype.cardinal = function (tag) {
        var s = this._tags[tag];
        if (s) return s.cardinal;
        return 0;
    };
    return TagSet;
}();
